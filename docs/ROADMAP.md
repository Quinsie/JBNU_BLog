...
| 1 | 수집기 (raw 무손실, 적응형+버스트금지) | ✅ 구현완료 (ITS 차단 쿨다운 중) | |
| 2 | 정적 기준 데이터 (노선·정류장·시간표·격자) | ✅ 완료 (446·격자43). route_nodes·traffic매핑은 **2차때**로 이월 | |
| 3 | **trip 재구성 + 클렌징** (raw→실제운행 ground truth) — *모델 무관* | 대기 | clean 데이터 축적 |
| 4 | **1차 모델 구조 설계 → feature 가공 → 학습** (사전 ETA, MLP vs GBDT) | 대기 | Phase 3 + 모델설계 |
| 5 | 2차 모델 (실시간 ETA, Transformer) + route_nodes/traffic 매핑 | 대기 | Phase 4 |
...
- **모든 후처리는 clean 데이터 축적이 전제.** (현재 5/26 오염 + 차단으로 clean 0)
- 데이터 쌓이는 동안 할 수 있는 것 = **설계**(trip 재구성/배차매칭 휴리스틱, 1차 모델 구조). ITS 불필요.
- route_nodes 는 **1차 모델과 무관**(2차 전용) → Phase 5 로 이월, 실효성 판단 후 결정.
