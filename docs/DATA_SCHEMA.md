...
`type` = `realtime`(초단기실황 getUltraSrtNcst) · `shortForecast`(초단기예보 getUltraSrtFcst) · `midForecast`(단기예보 getVilageFcst) · `longForecast`(중기예보, 별도 스키마 아래).
```jsonc
...
### raw/weather/longForecast/{YYYYMMDD_HH}.json  (중기예보, 6/18시 2회/일)
```jsonc
{ "ts","tmFc":"202605260600",   // 발표시각
  "land": {"ts","params","status","ok","body",...},  // getMidLandFcst(육상)
  "ta":   {"ts","params","status","ok","body",...} } // getMidTa(기온)
// 5/26 KMA 구독 반영 → land/ta ok=200 실데이터 수집 중. 단 BLog 가 4 격자만 호출.
```

### raw/incident/
ITS WAF 차단으로 **idle**. 코드(`src/collector/incident.py`)는 준비됨, `INCIDENT_URL` 미설정 시 1시간 sleep 루프. WAF 우회 작업은 v1 이후로 보류 → 현재 0 파일.
...
{ "stdid":305200112, "brt_no":"385", "plate_no":"2055",
  "company_name":"성진여객",                    // 운수사. trip 키=(plate,회사)(PLATE 4자리 비유일 보강) + 기사/차량 특성 feature
