...
- `base_time`은 첫 관측처럼 보이지만, 실제로는 `trackSingleBus.py`가 락온 시 **ORD 1 시각을 시간표 출발시각(`start_time_str`)으로 하드 고정**(+ORD 2 중간값 보간)했음. → *실제 출발이 아니라 시간표값으로 강제* = **인위적 오염**(이번 설계가 피하려는 핵심, §3.1).
- ⚠️ **작년 trip 추적은 `CURRENT_NODE_ORD` 기반**(락온 트리거·진행도·route_node 매칭). 반면 추론·mean은 `STOP_ORD` 기반 → **라벨 ord(node) vs 추론 ord(stop) 단위 불일치**(91% 우연 일치로 가려졌고 9%는 오정렬). vtx 수작업도 이 node↔geometry 매칭을 살리려던 것. → 이번엔 **STOP_ORD 일관 사용, node/geometry는 2차로 격리**.
...
- **형태 = 구간(정류장간) 예측 후 cumsum**.
  ...
  - **구간 vs 누적은 무손실 변환**: s_i=(도착 i)−(도착 i−1), y_k=Σs. 같은 타임스탬프의 차분/누적일 뿐. → **serving·평가지표는 "누적 도착 clock-time"으로 유지**(=목표 "발차→각 정류장 도착"), 구간은 *내부 학습표현*.
  - **오차누적 방어**: ① 평가는 누적으로 ② 출발 아는 trip엔 누적 일관성 손실(Σ예측구간 vs 실제누적) 병행 ③ 그래도 누적예측이 나으면 **누적 framing으로 회귀**(되돌릴 수 있는 실험).

### 2.5 열린 우려 (반복개선 시 검증 1순위)
- **[걱정 기록]** 사용자: 구간 라벨이 출발/누적과 "독립적"이라는 게 *논리는 이해되나 계속 걸린다*. 말로 떨어지지 않는 직관적 불안.
  - 검증 방법: 구간 framing 실제 학습 후, **누적 도착시각 MAE를 누적-직접예측 baseline과 정면 비교**. 구간합의 오차누적이 유의하게 나쁘면 §2.4 회귀. (= 이 우려를 측정으로 닫는다)
