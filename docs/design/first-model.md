...
### 3.1 결정 (확정)
- **앵커 = "log상 실제 출발이라 판단되는 시점"** (시간표 고정 ✗, 첫 관측 ✗).
  - 시간표 고정 → 11:45 배차가 실제 11:47/11:43 출발이어도 11:45로 뭉개짐 = **인위적 오염**(raw 받은 의미를 배신).
  - 첫 관측 고정 → 출발 30분 전 대기·상행기사 추적기 등 노이즈 못 거름.
  - → **실제 출발 검출(Phase 3 휴리스틱)이 1차 모델 y 라벨 자체를 정의**한다. 시간표는 앵커에서 강등 → *"이 trip이 몇시 배차였나"를 라벨링(배차간격 feature)* 용도로만.
- **형태 = 구간(정류장간) 예측 후 cumsum**.
  - 누적(출발→정류장 도착시각)은 cumsum으로 공짜로 따라옴 → 구간(지리·교통결합 자연) + 누적(agent용) 둘 다 획득.
  - 오차 국소화로 분산누적·이분산 완화, 희소노선이 구간 신호 차용.
  - **타깃 단위 = 정류장간 구간**(라벨 가능 단위), 그 구간이 지나는 **도로링크/교통**을 feature로 매핑(200m route-node는 feature 해상도, 타깃 ORD 유지).
  - 주의: 독립예측 단순합은 bias 누적 가능 → 합산 일관성 보정 후속 검토.
  - **라벨 수율 이점(실측)**: 구간 타깃이라 *중간진입 trip(5/26 실측 76%)도 구간 라벨로 유효*. 절대 출발앵커는 첫 구간·누적복원에만 필요 → 대부분 구간 라벨은 공짜. ([trip-reconstruction.md](trip-reconstruction.md) §5)
...
## 5. 갈림길 결정 (완료)
1. **y 앵커 = log상 실제 출발 검출 시점** (§3.1). → 출발검출 휴리스틱이 라벨의 전제.
2. **y 형태 = 구간(정류장간) 예측 + cumsum** (§3.1). 1·2차 granularity를 구간 단위로 통일.
3. **선행작업 전부 수용**: route_nodes·traffic매핑(작년 자산)을 1차 *전에* 올해 데이터로 재구성·재검증.

### 결정의 귀결 — 임계경로(critical path)
세 결정이 모두 **clean raw 데이터**를 전제로 함(현재 0: ITS 차단 + 5/26 오염). 의존 순서:
```
clean raw 축적 ──> Phase 3: 출발검출+trip재구성+클렌징 (= y 라벨 정의)
                └─> 기하 재검증: route_nodes 재생성 + 교통매핑 + vtx 보정
                                          └─> Phase 4: feature 가공 → 1차 학습
```
- 데이터 없이 지금 할 수 있는 설계: **출발검출/노이즈필터 휴리스틱**(작년 raw busPosList 시퀀스로 패턴 연구 가능), **기하 재검증 툴**(checkRoute 동등 + 육안 시각화 재작성).
